...
import { activateDeactivateUser, interviewsToday, getSearchUsers } from "_store/apis/userManagementAPI";
...
const AllUserManagement = () => {
  ...
  const [columnsNames] = useState(["Job Title" , "Interviewee" , "Date and Time" , "Round" , "Joining Link"]);
        ...
        <TableContainer className={styles.users_table_container}>
          <Table className={`${styles.users_table}`} aria-label="users table">
            ...
            <TableBody ref={tableBodyRef} className={styles.users_table_body}>
              {sportsData && sportsData.map(({ job ,  interviewee ,interviewDateTime , joiningLink, interviewRoundNumber  } : any , index) => (
